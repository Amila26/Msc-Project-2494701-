...
def get_api_key():
    ...
    api_key = "e61167b731db4a3381e102536230703"
    if not api_key:
...
def get_todays_weather_data(cities):
    ...
    todays_weather_df = pd.DataFrame()
# create Batch for the request limiting 50
    for batch in batches:
        city_parameters = [{"q": city, "custom_id": f"{slugify(city)}-request"} for city in batch]
        payload = json.dumps({
        ...
        batch_weather_df = pd.json_normalize(cities_weather_data)
        todays_weather_df = pd.concat([todays_weather_df, batch_weather_df])
...
def extract():
    ...
    # stores meta about historical sync - last synced (Handle Historical Data Update frequecny)
